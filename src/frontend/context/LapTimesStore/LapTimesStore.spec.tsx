...
describe('LapTimesStore', () => {
  ...
  it('should have initial state', () => {
    ...
    expect(state.playerCarIdx).toBeNull();
  });

  it('should set lapTimes to empty if no lap times', () => {
    useLapTimesStore.getState().updateLapTimes([], 1, 0);
  ...
  it('should use first lap time directly', () => {
    ...
    useLapTimesStore.getState().updateLapTimes([90.5], 1, 0);
    ...
    useLapTimesStore.getState().updateLapTimes([89.8], 1, 0);
    expect(useLapTimesStore.getState().lapTimes[0]).toBeCloseTo(90.15, 3);
  });

  it('should ignore invalid lap times', () => {
    useLapTimesStore.getState().updateLapTimes([90.5], 1, 0);
    useLapTimesStore.getState().updateLapTimes([0], 1, 0); // Invalid lap time
    expect(useLapTimesStore.getState().lapTimes[0]).toBeCloseTo(90.5, 3);
  });

  it('should ignore unchanged lap times', () => {
    useLapTimesStore.getState().updateLapTimes([90.5], 1, 0);
    useLapTimesStore.getState().updateLapTimes([90.5], 1, 0); // Same lap time
    expect(useLapTimesStore.getState().lapTimes[0]).toBeCloseTo(90.5, 3);
  });
  ...
  it('should filter outliers and use median for pace', () => {
    ...
    lapTimes.forEach((time) => {
      useLapTimesStore.getState().updateLapTimes([time], 1, 0);
    });
  ...
  it('should handle multiple cars with outliers', () => {
    ...
    useLapTimesStore.getState().updateLapTimes([90.5, 91.2], 1, 0);
    expect(useLapTimesStore.getState().lapTimes[0]).toBeCloseTo(90.5, 3);
    expect(useLapTimesStore.getState().lapTimes[1]).toBeCloseTo(91.2, 3);

    // Add some lap times including outliers
    useLapTimesStore.getState().updateLapTimes([89.8, 120.0], 1, 0); // 120.0 is an outlier
    useLapTimesStore.getState().updateLapTimes([90.2, 91.0], 1, 0);
    useLapTimesStore.getState().updateLapTimes([89.9, 91.1], 1, 0);
    useLapTimesStore.getState().updateLapTimes([90.1, 91.3], 1, 0);
  ...
  it('should auto-reset when session changes', () => {
    ...
    useLapTimesStore.getState().updateLapTimes([90.5], 1, 0);
    useLapTimesStore.getState().updateLapTimes([89.8], 1, 0);
    useLapTimesStore.getState().updateLapTimes([90.2], 1, 0);
    ...
    useLapTimesStore.getState().updateLapTimes([85.0], 2, 0);
  ...
  it('should handle null sessionNum gracefully', () => {
    useLapTimesStore.getState().updateLapTimes([90.5], null, 0);
    expect(useLapTimesStore.getState().lapTimes[0]).toBeCloseTo(90.5, 3);
