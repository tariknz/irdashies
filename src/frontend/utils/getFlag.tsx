...
export const getFlag = (sessionFlags: number) => {
  ...
  */
  ...
  if (sessionFlags & GlobalFlags.Checkered)
    return { label: 'CHECKERED', color: 'bg-white text-black' };
  if (sessionFlags & GlobalFlags.Red)
    return { label: 'RED', color: 'bg-red-600' };
  if (sessionFlags & GlobalFlags.Disqualify)
    return { label: 'DISQUALIFIED', color: 'bg-black text-red-600' };
  ...
  // 6. INFO FLAGS
  if (sessionFlags & GlobalFlags.Blue)
    return { label: 'BLUE FLAG', color: 'bg-blue-600' };
  if (sessionFlags & GlobalFlags.Debris)
    return { label: 'DEBRIS', color: 'bg-yellow-400' };
  if (sessionFlags & GlobalFlags.White)
    return { label: 'WHITE', color: 'bg-white text-black' };
