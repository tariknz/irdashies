...
describe('getFlagInfo', () => {
  ...
  describe('Track Cautions', () => {
    it('should return YELLOW flag for various caution bits', () => {
      expect(getFlag(GlobalFlags.Yellow).label).toBe('YELLOW');
      expect(getFlag(GlobalFlags.YellowWaving).label).toBe('YELLOW');
      expect(getFlag(GlobalFlags.Caution).label).toBe('YELLOW');
      expect(getFlag(GlobalFlags.CautionWaving).label).toBe('YELLOW');
    });
  ...
  describe('Info Flags', () => {
    it('should follow internal priority: BLUE > DEBRIS > WHITE', () => {
      expect(getFlag(GlobalFlags.Blue | GlobalFlags.Debris).label).toBe('BLUE FLAG');
      expect(getFlag(GlobalFlags.Debris | GlobalFlags.White).label).toBe('DEBRIS');
    });
  }); 
  ...
  describe('Priority Ordering & Edge Cases', () => {
    it('should return BLACK FLAG when all bits are set (Absolute Priority)', () => {
      ...
      const result = getFlag(allFlags);
      expect(result.label).toBe('BLACK FLAG');
    });

    it('should return NO FLAG when no relevant bits are set', () => {
      expect(getFlag(0).label).toBe('NO FLAG');
      expect(getFlag(GlobalFlags.OneLapToGreen).label).toBe('NO FLAG');
    });
  });
});