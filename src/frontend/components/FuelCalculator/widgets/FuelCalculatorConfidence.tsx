...
export const FuelCalculatorConfidence: React.FC<FuelCalculatorWidgetProps> = ({ fuelData, settings, widgetId, customStyles, isCompact }) => {
    ...
    const containerPadding = isCompact ? 'py-0.5 px-2 mb-0.5' : 'py-1.5 px-2 mb-2';

    if (confidence === 'low') {
        return (
            <div className={`${containerPadding} bg-red-500/10 border border-red-500/30 rounded`}>
    ...
    if (confidence === 'medium') {
        return (
            <div className={`${containerPadding} bg-amber-500/10 border border-amber-500/30 rounded`}>
    ...
    // High confidence
    return (
        <div className={`${containerPadding} bg-green-500/5 border border-green-500/20 rounded`}>
            <div className="text-center" style={{ fontSize: valueFontSize }}>
                <span className="text-slate-400">Fuelling for <span className="text-white font-medium">{fuelForLaps}</span> laps (worst case)</span>
            </div>
        </div>
