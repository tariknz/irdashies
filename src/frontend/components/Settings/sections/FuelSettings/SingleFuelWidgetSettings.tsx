...
import { FuelWidgetSettings, getWidgetDefaultSettings } from '@irdashies/types';
...
export const SingleFuelWidgetSettings = ({
  ...
  const { settings, setSettings, activeTab, setActiveTab } =
    useWidgetSettingsSection('fuel', widgetId, () => {
      const savedConfig =
        (savedSettings?.config as FuelWidgetSettings['config']) ??
        defaultConfig;
      ...
      return {
        ...
        config: needsDefaultTree
          ...
          : savedConfig,
        visibilityConfig: getWidgetDefaultSettings('fuel').visibilityConfig,
      };
    });
