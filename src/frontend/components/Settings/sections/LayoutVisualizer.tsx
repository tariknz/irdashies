...
import { LayoutNode, BoxConfig } from '@irdashies/types';
...
export const migrateToTree = (flatConfig: BoxConfig[]): LayoutNode => {
      ...
      children: [
        {
          id: generateId(),
          type: 'box',
          widgets: ['fuelLevel', 'lapsRemaining'],
          direction: 'row',
          weight: 1,
        },
        {
  ...
  const children: LayoutNode[] = flatConfig
    .map((box) => {
  ...
  if (node.type === 'box') {
    return (
      ...
      >
        ...
        <div
          className={`rounded bg-slate-900/50 p-1 flex-1 flex gap-2 overflow-hidden ${node.direction === 'row' ? 'flex-row' : 'flex-col'}`}
        >
          {node.widgets?.map((wId, idx) => {
            const isTargetWidget =
              dragState.targetId === node.id &&
              dragState.targetWidgetIdx === idx;
            const isReordering =
              dragState.targetId === node.id && dragState.dropZone === 'center';
              ...
              >
                ...
                >
                  <span className="text-xs text-slate-200 truncate">
                    {getWidgetLabel(wId, availableWidgets)}
                  </span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onRemoveWidgetFromBox(node.id, idx);
                    }}
          ...
          {(!node.widgets || node.widgets.length === 0) && (
            <span className="text-xs text-slate-500 italic p-1">Empty</span>
          )}
        </div>
  ...
  const removeNodeById = (
    ...
    if (node.type === 'split') {
      const newChildren = node.children
        .map((c) => removeNodeById(c, idToRemove))
        .filter(Boolean) as LayoutNode[];
  ...
  const insertNode = (node: LayoutNode | null): LayoutNode => {
    ...
    if (node.type === 'split') {
      ...
      if (targetIdx !== -1) {
        ...
        const splitNode: LayoutNode = {
          ...
          direction,
          children: isAfter
            ? [childToSplit, boxToInsert]
            : [boxToInsert, childToSplit],
    ...
    if (node.id === targetNode.id) {
      ...
      if (zone === 'center' && node.type === 'box') {
        // Merge widgets
        const widgetsToAdd =
          draggedNode.type === 'box' ? draggedNode.widgets : [];
        const finalWidgets = [...node.widgets];
        widgetsToAdd.forEach((w) => {
          if (!finalWidgets.includes(w)) finalWidgets.push(w);
        });
...
export const LayoutVisualizer = ({
  ...
  const handleDrop = (e: React.DragEvent, targetNode: LayoutNode) => {
    e.preventDefault();
    e.stopPropagation();
    const { draggedNode, dropZone, sourceBoxId, widgetIdx, targetWidgetIdx } =
      dragState;
    ...
    if (
      dropZone === 'center' &&
      targetNode.type === 'box' &&
      (draggedNode as LayoutNode & { type: 'box' }).widgets?.length === 1
    ) {
      const sourceWidgetId = (draggedNode as LayoutNode & { type: 'box' })
        .widgets[0];
      const updateTarget = (node: LayoutNode): LayoutNode => {
        ...
        if (node.type === 'split')
          return { ...node, children: node.children.map(updateTarget) };
        return node;
      };
  ...
  return (
    <div className="flex gap-4 h-[450px]">
      ...
      {/* Sidebar Pool */}
      <div
        className={`w-48 bg-slate-800 border box-border rounded p-3 flex flex-col gap-3 overflow-y-auto transition-colors ${
        ...
        onDrop={(e) => {
          ...
          if (dragState.sourceBoxId !== null && dragState.widgetIdx !== null) {
            handleRemoveWidgetFromBox(
              dragState.sourceBoxId,
              dragState.widgetIdx
            );
          }
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {dragState.draggedNode && (
            <TrashIcon className="text-red-400 animate-pulse" weight="fill" />
          )}
        </div>
