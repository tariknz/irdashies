...
export const SessionBar = ({
  ...
  return (
    <div
      className={`${pxClass} ${pyClass} flex items-center text-sm ${standalone ? `w-full justify-between ${gapClass}` : 'bg-slate-900/70 justify-between'} ${!isCompact && !isUltra && !standalone ? (position === 'header' ? 'mb-3' : 'mt-3') : ''}`}
    >
      {itemsToRender}
    </div>
  );
