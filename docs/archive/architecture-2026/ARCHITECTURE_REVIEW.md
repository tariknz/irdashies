...
> **Archived:** This is a historical record of the 2026 architecture programme. The programme is closed; this document is not an active proposal or task list. Current requirements live in [`../../ARCHITECTURE_RULES.md`](../../ARCHITECTURE_RULES.md).
>
> **Status at time of writing:** Proposal (revised after Phase 0 measurement) — May 2026
> **Audience:** Maintainers and LLM coding agents working on the codebase
> **Companion files:**
>
> - [`ARCHITECTURE_RULES.md`](../../ARCHITECTURE_RULES.md) — the current enforceable rules derived from this review
...
2. **Architectural debt around lifecycle, coupling, and god-files** — `Standings/` has become an unofficial domain library that 6+ other widgets reach into; the disconnect-reset code path is dead (_resolved in Phase 1 — see A3_); the multi-profile system is bypassed by a hardcoded `'default'` profile; adding a widget edits 5+ god-files.
...
| **A2** | _(as audited, 2026-05)_ No `SessionLifecycle` abstraction — each store invents its own session-change detector | [LapTimesStoreUpdater.tsx:22](../../../src/frontend/context/LapTimesStore/LapTimesStoreUpdater.tsx#L22), [SectorTimingStore.tsx:250](../../../src/frontend/context/SectorTimingStore/SectorTimingStore.tsx#L250), `PitLapStore`, `ReferenceLapStore` | **Now confirmed as a primary leak source.** Stores drift; resets miss; replays look like new laps; _and_ — per Practice 2 — there is no centralised handler for "driver joined / driver left" that bridges and stores can hook into, which is the mechanism driving P7 | **SUBSTANTIALLY RESOLVED:** `src/app/sessionLifecycle/` now provides enter, driver-join, driver-leave, session-number-change, and disconnect events; synthetic disconnect leaves and cleanup landed in Phase 2a and were validated. The remaining gap is migrating every session-derived store to this source and retiring store-specific detectors — see [IMPLEMENTATION_PLAN.md](./IMPLEMENTATION_PLAN.md) Phase 2b |
| **A3** | _(as audited, 2026-05)_ `useResetOnDisconnect` has zero callers and omits 3 stores | [useResetOnDisconnect.ts](../../../src/frontend/context/shared/useResetOnDisconnect.ts) | Dead code; reference laps and sector timing leak across sessions. **Now reclassified:** _not_ implicated as the in-race steady-state leak source (single-driver Test 1 leaks at +19.7 MB/min with no reference data), _but_ CONFIRMED implicated for the driver-join leak (P7) — the absence of a reset path matches the observed pattern exactly: state added on join, nothing to release it on leave. The team has shipped a one-time `migrateReferenceLaps()` cleanup ([referenceLaps.ts:15-37](../../../src/app/storage/referenceLaps.ts#L15-L37), 2026-05 release) which removes stale on-disk data once but does not address the missing in-session reset path | **CONFIRMED** implicated for driver-join leak; **DISCONFIRMED** as in-race steady-state leak source. **PARTIALLY RESOLVED (Phase 1):** `useResetOnDisconnect(running)` is activated in [OverlayContainer.tsx:27](../../../src/frontend/components/OverlayContainer/OverlayContainer.tsx#L27), so it is no longer dead code; the stores it still omits are tracked under Phase 2b in [IMPLEMENTATION_PLAN.md](./IMPLEMENTATION_PLAN.md) |
...
4. **Backwards compatibility window — resolved by PR #671.** All in-tree
   widgets have migrated. The generic legacy telemetry provider is removed; raw
   telemetry is available only through the explicit, demand-driven Telemetry
   Inspector stream.
