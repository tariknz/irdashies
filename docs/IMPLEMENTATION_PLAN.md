...
| **Phase 4 — Main-process processors**                 | LANDED; PERF VALIDATION PENDING | PRs #659–#671                            | All normal widgets use typed snapshots. PR #671 removed the generic renderer telemetry provider and retained raw telemetry only behind the demand-driven 10 Hz Telemetry Inspector bridge. Phase 4.1 now addresses contract, lifecycle, duplication, and benchmark gaps before the packaged performance re-profile.                                                       |
