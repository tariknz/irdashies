...
| **Phase 4 — Main-process processors**                 | LANDED; MEMORY GATE OPEN | PRs #659–#671                            | All normal widgets use typed snapshots on `main`; arbitrary raw inspection is isolated behind a demand-driven 10 Hz Telemetry Inspector bridge. Packaged Windows A/B passes CPU, cadence, latency, and frame-pacing gates, but working memory grows +7.63 MB/min and private memory +10.47 MB/min. Incremental retention is concentrated in the main process.             |
...
- [x] Direct telemetry migration and legacy removal
  ...
  4. [x] Replace the generic renderer telemetry IPC/provider with a dedicated demand-driven 10 Hz Telemetry Inspector bridge — merged as PR #671
