...
7. **Single-widget isolation tests** _(CAPTURED 2026-07-26)_ — Standings, Fuel, and Relative were isolated with the real overlay container. Active-renderer CPU was 0.75%, 0.74%, and 0.57%, respectively. Fuel showed the clearest short-run allocation signal (+22.01 MB/min active-renderer private memory); the other two windows crossed major GCs, so longer allocation profiles are required before treating slopes as rankings.
