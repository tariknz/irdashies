...
| **Phase 4 — Main-process processors**                 | IN PROGRESS              | PRs #659–#669; `feat/remaining-telemetry-channels` | The planned derived processors, complete Session Bar migration, driver controls, and positional consumers are on `main`. The remaining-consumer branch moves normal widgets off raw telemetry and reserves the legacy stream for Telemetry Inspector; final infrastructure deletion and re-profiling remain.                                                              |
...
- [ ] Direct telemetry migration and legacy removal
  ...
  3. [ ] Remaining low-frequency consumers plus an explicit development-only path for Telemetry Inspector — `feat/remaining-telemetry-channels` in progress
