...
| **Phase 4 — Main-process processors**                 | FINAL CLEANUP IN PROGRESS | PRs #659–#670; `feat/telemetry-inspector-debug-bridge` | All normal widgets use typed snapshots on `main`. The final cleanup isolates arbitrary raw inspection behind a demand-driven 10 Hz Telemetry Inspector bridge, removes the generic renderer telemetry API/provider path, and prepares the final replay and performance re-profile.                                                                                        |
...
- [ ] Direct telemetry migration and legacy removal
  ...
  4. [ ] Replace the generic renderer telemetry IPC/provider with a dedicated demand-driven 10 Hz Telemetry Inspector bridge, run the complete replay suite, and re-profile — `feat/telemetry-inspector-debug-bridge` in progress
